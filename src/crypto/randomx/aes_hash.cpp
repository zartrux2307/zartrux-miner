...
void SelectSoftAESImpl(size_t threadsCount)
{
  ...
  for (size_t run = 0; run < 3; ++run) {
    for (size_t i = 0; i < impl.size(); ++i) {
      const double t1 = zartrux::Chrono::highResolutionMSecs();
      ...
      for (size_t t = 0; t < threadsCount; ++t) {
        threads.emplace_back([&, t]() {
          ...
          ++count[t];
          } while (zartrux::Chrono::highResolutionMSecs() - t1 < test_length_ms);
        });
      }
      uint32_t total = 0;
      for (size_t t = 0; t < threadsCount; ++t) {
      ...
      const double t2 = zartrux::Chrono::highResolutionMSecs();
